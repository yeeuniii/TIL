...
### 왜 OLTP는 테이블을 쪼개고, OLAP는 테이블을 하나로 합칠까?
OLTP와 정규화: 중복을 피하자!  
OLTP 환경에서 중요한 것은 데이터의 무결성이다.   
테이블을 쪼개는(join, 정규화를 하는) 이유는, 
- 데이터 무결성 유지
- 쓰기 성능 최적화
- 저장 공간 절약   

예를 들어, 이름 컬럼이 여러 테이블에 중복되어 저장되어 있는 경우, 특정 인물의 이름이 변경되면 여러 테이블의 데이터를 모두 변경해야 하고, 이름 컬럼 데이터 추가 시에도 여러 테이블에 모두 데이터를 저장해야 함. 
  
OLAP와 비정규화: 조인은 사치다.   
OLAP 환경에서 중요한 것은 빠른 조회 속도이다. 
테이블을 합치는(wide table, 비정규화 하는) 이유는,
- 조인(join) 비용 제거
- 읽기 성능 극대화 
- 컬럼 기반 DB와의 시너지. 컬럼이 많아져도 부담이 없음. 


#### Q. 게임 유저 로그(fastlog)가 하나의 테이블에서 모든 이벤트 그룹을 저장하는 이유가 OLAP 환경이기 때문?
상황: 
- 하나의 테이블에 모든 이벤트 그룹의 데이터를 모두 저장. 
- eg 파티션으로 분리. 
- 같은 컬럼이지만, eg에 따라서 다른 속성을 가짐. 

분산 환경에서 수억건의 데이터를 join하는 것은 네트워크 부하 및 메모리 소모가 크다. 
OLAP 환경에서는 쿼리 타임에 join하는 것보다, 데이터를 적재할 때 미리 join하는 것을 지향한다.
